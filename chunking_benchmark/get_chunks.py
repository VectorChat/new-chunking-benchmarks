...
from chunking_benchmark.utils.chunks import (
    get_chunk_stats,
    print_chunk_stats,
    save_chunk_size_hist_from_json,
)
...
argparser.add_argument(
    "--vali_wallet",
    type=str,
    help="Coldkey for validator wallet to query individual miners",
)
argparser.add_argument(
    "--vali_hotkey",
    type=str,
    help="Hotkey for validator wallet to query individual miners",
)
...
argparser.add_argument(
    "--text_file", type=str, required=True, help="Path to text file or directory"
)
argparser.add_argument(
    "--miner_uids", type=str, help="Comma separated list of miner uids to query"
)
...
async def main():
    ...
    text_files = (
        glob(os.path.join(args.text_file, "*.txt"))
        + glob(os.path.join(args.text_file, "*.md"))
    ...
    for text_file in tqdm(text_files):
        ...
        for chunk_runner in chunk_runners:
            ...
            out_path = os.path.join(
                args.out_dir, date_str, chunk_runner.name, f"{chapter}.json"
            )
            os.makedirs(os.path.dirname(out_path), exist_ok=True)
            chapter_chunk = [
                {"text": chunk, "chapter_name": chapter} for chunk in chunks
            ]
            ...
            res[chunk_runner.name] = (
                chapter_chunk
                if chunk_runner.name not in res
                else res[chunk_runner.name] + chapter_chunk
            )
            metas[chunk_runner.name] = (
                [metadata]
                if chunk_runner.name not in metas
                else metas[chunk_runner.name] + [metadata]
            )
            print(
                tabulate(
                    table_rows,
    ...
    for k, v in res.items():
        ...
        os.makedirs(os.path.dirname(out_path), exist_ok=True)
        with open(out_path, "w") as f, open(
            out_path.replace(".json", "_metadata.json"), "w"
        ) as f_meta:
