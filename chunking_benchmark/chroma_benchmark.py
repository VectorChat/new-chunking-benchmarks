...
from retrievers.embedder import HFEmbedder, OpenAIEmbedder
...
@joblib.Memory(location="./cachedir_tmp", verbose=0).cache(ignore=["embedder", "db_client"])
def retrieve_relevant_chunks(
    ...
    **kwargs,
    ...
    # rerank chunks
    reranker = AWSReranker()  # CohereReranker()
...
def get_dataset(path_dataset: str = "assets/chroma/questions.csv", num_questions: int = 2) -> pd.DataFrame:
    # chroma dataset: question,references,corpus_id
    df = pd.read_csv(path_dataset, nrows=num_questions)
...
def run_single_benchmark(
    ...
    **kwargs,
    ...
    df = get_dataset(num_questions=num_questions)
...
async def main(args):
    ...
    out_dir = f"assets/chroma/results/{date_str}"
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--index_name",
        type=str,
        default="openai-index-chunking-benchmark-chroma-tmp",
        help="Index name for the vector database",
    )
    parser.add_argument(
        "--topk_retrieval",
        type=int,
        default=400,
        help="Top k chunks to retrieve from vector database",
    )
    parser.add_argument(
        "--topk_rerank",
        type=int,
        default=100,
        help="Top k chunks to rerank",
    )
    parser.add_argument(
        "--token_limit",
        nargs="+",
        type=int,
        help="Token limit for the context retrieval",
        default=[50, 150, 250],
    )
    parser.add_argument(
        "--num_questions",
